...
function Lecture() {
  ...
  const [lecid, setLecId] = useState(
    `${lectureid}+${generateRandomString(20)}`,
  );
  ...
  useEffect(() => {
    async function request() {
      try {
        setLoading(true);
        const skt = io(`${import.meta.env.VITE_API_URL}/`);
        setSocket(skt);
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/lecture/${lectureid}`,
          {
            headers: myHeaders,
          },
        );
        const response2 = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/singleclass/${classid}`,
          {
            headers: myHeaders,
          },
        );
    ...
    <div className="LAT_WRAPPER">
      <section className="LAT_mainPage">
        ...
        <div className="LAT_QRcontainer">
          <div className="LAT_detailsContainer">
            ...
            <div className="LAT_details">
              <p>
                Date: <span>{formatDate(lectureData.dateTime)}</span>
              </p>
              <p>
                Time: <span>{formatTime(lectureData.dateTime)}</span>
              </p>
            </div>
          </div>
          <QRCodeSVG className="QRcode" value={lecid} />
        ...
        <section className="LAT_table">
          ...
          {lectureData.attendance &&
            lectureData.attendance.map((stud, index) => (
